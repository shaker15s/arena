...
## 7) تحديث 2026-09-06 — التجاوب والعربية

**منفَّذ ومُتحقَّق منه (`test:all` 79 ✅ / `export:web` ✅):**

- **سلم تايبوغرافيا متجاوب** — `uiScale(width)` و`scaleType()` في `tokens.ts`، مطبَّقان في نقطة واحدة (`Txt`) فيسريان على كل الشاشات:
  - القاعدة 390pt، الحدود **0.92–1.08** فقط (تصحيح محسوس بلا كسر تخطيط).
  - **تقريب لأعداد صحيحة** — القيم الكسرية تجعل رسم الحروف العربية يهتز ويقصّ الامتدادات على أندرويد.
  - **نسبة `lineHeight` محفوظة** لا مُقلَّصة مستقلة — شرط أساسي لسلامة الامتدادات العربية.
  - حد أدنى مطلق 11px.
- **قصّ النص العربي** — `Txt` يفعّل `adjustsFontSizeToFit` تلقائيًا عند `numberOfLines === 1` بحد `minimumFontScale=0.85`: يصغّر بدل ما يبتر. أُضيف prop `shrink` للتحكم الصريح.
- **وصولية** — 9 عناصر تفاعلية كانت صامتة أخذت `accessibilityLabel` عربية من `t()`، ومجموعات الاختيار (النوع/الفرع) أخذت `accessibilityRole="radio"` مع حالة الاختيار، وأهداف اللمس الصغيرة رُفعت إلى `sizes.touchTarget`.

**لا يزال مؤجَّلًا (يحتاج جهازًا حقيقيًا):**
- فحص بصري على أندرويد فعلي: 360/390/430pt × تكبير خط 100%/140%.
- قياس FPS على شاشة الدوري.
- `FlatList`: القرار كما هو — عند أول jank مقيس، لا استباقيًا.
